...
###########################################################
# Modified by RikedyP 
# Date is now YYYY.MM.DD for easier sorting by python sorted
...
class DatePicker(TextInput):
    ...
    def init_ui(self):
        ...
        self.text = str(cal_data.today_date_list())
        # Calendar
        ...
        self.bind(focus=self.show_popup) # When you click it the popup pops up
...
class CalendarWidget(RelativeLayout):
    ...
    def create_month_scr(self, month, toogle_today=False):
        ...
        scr.name = "%s-%s" % (m, self.active_date[0])  # like march-2015
        # Grid for days
        ...
        for week in month:
            for day in week:
                ...
                if toogle_today:
                    # Down today button
                    if day[0] == self.active_date[2] and day[2] == 1:
                        tbtn.state = "down"
    ...
    def get_quarter(self):
        ...
        self.quarter_nums = cal_data.calc_quarter(self.active_date[0], 
                                                  self.active_date[1])
        self.quarter = cal_data.get_quarter(self.active_date[0], 
                                            self.active_date[1])
    
    def get_btn_value(self, inst):
        """ Get day value from pressed button """
        
        self.active_date[2] = int(inst.text)
    ...
    def go_prev(self, inst):
        ...
        # Change active date
        self.active_date = [self.quarter_nums[0][0], self.quarter_nums[0][1], 
                            self.active_date[2]]
    ...
    def go_next(self, inst):
        """ Go to screen with next month """
         ...
         # Change active date
        self.active_date = [self.quarter_nums[2][0], self.quarter_nums[2][1], 
                            self.active_date[2]]
        # Name of prev screen
